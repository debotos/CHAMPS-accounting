...
export class Equipment extends Component {
	...
	_bootstrap = async budgetYear => {
		try {
			this.setState({ loading: true })
			const response = await axios.get(`/api/v1/equipment?budgetYear=${budgetYear}`)
			const data = response.data.map(x => ({ ...x, key: x._id })).reverse()
			const budget = await this.getBudget()
			const bank = await this.getBank()
			this.setState({ data, budget: budget.equipment, bank: bank.equipment }, () =>
